...
export default function AppPage() {
  ...
  useEffect(() => {
    (async () => {
      if (mode === 'demo_pin') {
        setJobs([{ id: 'demo-1', address_line1: '11 Acacia Avenue', region: 'Gloucester', postcode: 'GL1 2AB', order_no: 'ORD-77819' }]);
        setActiveJobId('demo-1');
        setItems([
          { id: 'i1', code: '7300EA', description: 'VOID:SAFETY CHECK AND TEST GAS INSTALLATION', base_rate_pence: toPence(58.01), contract_rate_pence: toPence(65.55), unit: 'IT', qty: 1, total_pence: toPence(65.55), location: 'General', comments: 'Landlord gas & safety certificate', category: 'Gas', assignee_name: 'Bristol Boilers', status: 'Assigned' },
          { id: 'i2', code: 'NONSOR', description: 'CARPET:RENEW TO DOMESTIC AREAS', base_rate_pence: toPence(27.91), contract_rate_pence: toPence(32.10), unit: 'IT', qty: 2, total_pence: toPence(64.20), location: 'Lounge', comments: 'Fixed-rate special', category: 'Flooring', assignee_name: '', status: 'Unassigned' },
        ]);
      ...
      if (!error && jobsData) {
        setJobs(jobsData);
        if (jobsData[0]) setActiveJobId(jobsData[0].id);
      }
    })();
  }, [mode]);
  ...
  useEffect(() => {
    if (!activeJob) return;
    if (mode === 'demo_pin') return; // demo uses local items
    (async () => {
  ...
  async function handleUpload(file: File, property: any) {
    ...
    if (mode === 'demo_pin') {
      ...
      const { data: jobIns, error } = await supabase.from('jobs').insert([{ ...property }]).select('id').single();
    ...
    // Fetch categories + rules + regional defaults (DB modes)
    let categories: any[] = seedCategories.map((n, i) => ({ id: `local-${i}`, name: n }));
    let rules: any[] = [{ kind: 'NONSOR', pattern: 'CARPET:RENEW TO DOMESTIC AREAS', categoryName: 'Flooring', priority: 1 }];
    let defaults: Record<string, string> = {};

    if (mode !== 'demo_pin') {
      const [cat, rule, def] = await Promise.all([
        supabase.from('categories').select('*'),
        supabase.from('category_rules').select('*, categories:category_id(name)'),
        supabase.from('regional_defaults').select('*, categories:category_id(name), contractors:contractor_id(company_name)')
      ]);
      if (!cat.error && cat.data) categories = cat.data;
      if (!rule.error && rule.data) rules = rule.data.map((r: any) => ({ kind: r.kind, pattern: r.pattern, categoryName: r.categories?.name, priority: r.priority }));
      if (!def.error && def.data) {
        defaults = Object.fromEntries(def.data.map((d: any) => [`${d.categories?.name}|${d.region}`, d.contractors?.company_name]));
      }
    }

    const norm = (s: string) => (s || '').trim();
    function classify(row: SheetRow): { category: string; needsReview: boolean } {
      // exact code rules first
    ...
    if (mode === 'demo_pin') {
      setItems(toInsert.map((x, i) => ({ id: `demo-item-${Date.now()}-${i}`, ...x })));
  ...
  async function addEvidence(id: string, file: File) {
    if (mode === 'demo_pin') { alert('Demo mode: not uploading.'); return; }
    const path = `${id}/${Date.now()}-${file.name}`;
    const { error: upErr } = await supabase.storage.from('evidence').upload(path, file, { upsert: false });
    if (upErr) return alert(upErr.message);
    const { error: dbErr } = await supabase.from('evidence').insert([{ item_id: id, type: 'photo', file_path: path, filename: file.name, filesize_bytes: file.size }]);
  ...
  const completeValue = useMemo(() => items.filter(r => r.status === 'Completed').reduce((s, r) => s + r.total_pence, 0), [items]);
  const totalValue = useMemo(() => items.reduce((s, r) => s + r.total_pence, 0), [items]);
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-6 grid grid-cols-1 lg:grid-cols-[240px_1fr] gap-6">
      ...
      <section className="space-y-6">
        {route === 'jobs' && (
          <Card title="Jobs" subtitle={`${jobs.length} job(s)`} actions={<Button variant="outline" onClick={()=>setRoute('upload')}>New upload</Button>}>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {jobs.map((j:any) => (
                <Card key={j.id} title={j.address_line1 || '(no address)'} subtitle={`${j.region || '—'} • ${j.postcode || '—'}`} actions={<Button size="sm" onClick={()=>{ setActiveJobId(j.id); setRoute('items'); }}>Open</Button>}>
                  <div className="text-sm text-gray-600">Order: {j.order_no || '—'}</div>
                </Card>
              ))}
            </div>
          </Card>
          ...
          <div className="space-y-4">
            ...
            <Card title="Property details" subtitle="All fields optional">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                {['inspected_by','address_line1','address_line2','town','postcode','key_location','key_safe_code','order_no','property_type','tenure_type','condition','uprn','fund','client_name'].map(k => (
                  <Input key={k} value={property[k]||''} onChange={v=>setProperty((p:any)=>({...p,[k]:v}))} placeholder={k.replaceAll('_',' ').replace(/\b\w/g, c=>c.toUpperCase())} />
                ))}
                <Select value={property.region||''} onChange={v=>setProperty((p:any)=>({...p,region:v}))}>
                  {['Gloucester','Hackney','Bristol'].map(r => <option key={r}>{r}</option>)}
                </Select>
              </div>
            </Card>
            <div className="flex items-center justify-end gap-2">
              <Button variant="outline" onClick={()=>{setFile(null); setProperty({ region: 'Gloucester' });}}>Reset</Button>
              <Button onClick={()=>{ if(!file) return alert('Choose a file first'); handleUpload(file, property); }}>Create Job</Button>
            </div>
          </div>
        )}
        ...
        {route === 'items' && (
          <div className="space-y-4">
            <Card title="Job overview" subtitle={`${activeJob?.address_line1 || '(no address)'} — ${activeJob?.region || '—'}`} actions={<>
              <Badge tone="blue">{Math.round((items.filter(r=>r.status==='Completed').length / Math.max(items.length,1))*100)}% complete</Badge>
              <Badge tone="gray">{pounds(fromPence(completeValue))} / {pounds(fromPence(totalValue))}</Badge>
              <Button variant="outline" onClick={exportCurrent}>Export</Button>
            </>}>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-sm">
                <div><div className="text-gray-500">Order No.</div><div className="font-medium">{activeJob?.order_no || '—'}</div></div>
                <div><div className="text-gray-500">UPRN</div><div className="font-medium">{activeJob?.uprn || '—'}</div></div>
                <div><div className="text-gray-500">Key Safe</div><div className="font-medium">{activeJob?.key_safe_code || '—'}</div></div>
                <div><div className="text-gray-500">Postcode</div><div className="font-medium">{activeJob?.postcode || '—'}</div></div>
              </div>
            </Card>
                ...
                <table className="min-w-[960px] w-full text-sm">
                  <thead className="bg-gray-50 text-gray-600">
                    <tr className="text-left">
                      {['Code','Description','Base','Contract','Unit','Qty','Total','Location','Comments','Category','Assignee','Status','Evidence'].map(h => (
                  ...
                  <tbody>
                    {items.map((row:any) => (
                      <tr key={row.id} className="border-t">
                        ...
                        <td className="px-3 py-2 max-w-[380px]"><div className="truncate" title={row.description}>{row.description}</div></td>
                        <td className="px-3 py-2">{pounds(fromPence(row.base_rate_pence))}</td>
                        <td className="px-3 py-2">{pounds(fromPence(row.contract_rate_pence))}</td>
                        <td className="px-3 py-2">{row.unit}</td>
                        <td className="px-3 py-2">{row.qty}</td>
                        <td className="px-3 py-2 font-medium">{pounds(fromPence(row.total_pence))}</td>
                        <td className="px-3 py-2">{row.location}</td>
                        <td className="px-3 py-2 max-w-[260px] truncate" title={row.comments}>{row.comments || '—'}</td>
                        <td className="px-3 py-2">
                          <Select value={row.category} onChange={v => updateItem(row.id, { category: v })}>
                            {seedCategories.map(c => <option key={c}>{c}</option>)}
                          </Select>
                        </td>
                        <td className="px-3 py-2">
                          <Input value={row.assignee_name || ''} onChange={v => updateItem(row.id, { assignee_name: v })} placeholder="Contractor name" />
                        </td>
                        <td className="px-3 py-2">
                          <Select value={row.status} onChange={v => updateItem(row.id, { status: v })}>
                            {['Unassigned','Assigned','Started','Completed'].map(s => <option key={s}>{s}</option>)}
                          </Select>
                          <div className="mt-1"><Badge tone={statusTone(row.status)}>{row.status}</Badge></div>
                        </td>
                        <td className="px-3 py-2">
                          <input type="file" accept="image/*,application/pdf" onChange={e=>{ const f=e.target.files?.[0]; if(f) addEvidence(row.id, f); }} />
                        </td>
                      </tr>
                    ))}
                  </tbody>
